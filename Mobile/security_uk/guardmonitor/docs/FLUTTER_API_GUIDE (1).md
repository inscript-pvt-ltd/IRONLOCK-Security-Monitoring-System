...
**Offline (schedule-triggered):** while online, keep a **pool of nonces** topped up (`POST /shifts/{id}/nonces/prefetch`). Each pool nonce is single-use and valid **15 min**. The **trigger** for an offline capture is the **photo schedule** provisioned at shift start (below) — exactly like the wakefulness schedule. When a schedule mark comes due **while offline**, draw a pool nonce, capture, queue, and upload when connectivity returns — **omit `request_id`**, just send `nonce_value`. The server reconstructs the capture time and accepts if it's within the nonce's 15-min window. (While **online**, the same mark instead arrives as a pushed `PHOTO_REQUEST`, so don't also self-fire — the schedule is one source of truth for both paths.)

### Provisioning — the photo schedule (`POST /shifts/{id}/start`)

`POST /shifts/{id}/start` returns a `photos` block alongside `wakefulness`: the randomised due-times the server will check against, plus the window parameters. Store the schedule and, **while offline**, fire the camera at each mark (mirror your `wakefulness` scheduler):

```jsonc
"photos": {
  "schedule": [                       // ISO-8601 UTC due-times — same shape as wakefulness.schedule
    "2026-06-30T12:28:00Z",
    "2026-06-30T13:18:00Z"
  ],
  "response_seconds": 90,             // ONLINE window only (a pushed request); NOT used offline
  "offline_nonce_ttl_minutes": 15,    // offline: the drawn pool nonce is valid this long; capture must fall inside
  "max_photos_per_capture": 5         // up to 5 images per mark (photos[]/signatures[])
}
```

- **No per-mark id and no offline response countdown.** Offline validity is purely the 15-min pool-nonce window — there's no 90 s timer offline. The server matches a fired check to a schedule mark by timestamp (±60 s tolerance), so you don't echo a mark id.
- A schedule-triggered offline photo lands as its **own** evidence record on the shift timeline (`request_type = SCHEDULED`, no admin requester) — it is **not** tied back to any online `request_id`.
...
**Rejected `422` `PHOTO_REJECTED`** (not stored): branch on `error.details.reason` — `NONCE_NOT_FOUND`, `NONCE_ALREADY_USED`, `NONCE_WRONG_SHIFT`, `NONCE_EXPIRED`, `TIMELINE_ANOMALY`, `HMAC_INVALID`, `REQUEST_NOT_FOUND`. On `HMAC_INVALID`, re-login to refresh `hmac_secret` then retry. `NONCE_WRONG_SHIFT` means the nonce was issued for a different shift than the one in the URL — **do not retry**; upload it to the shift the nonce belongs to (the shift that was active when it was prefetched).
...
> **Foreground vs background (the "screen on, no prompt" fix).** When the app is
> **already open** (foreground), do **not** wait for the guard to tap a tray
> notification — raise the code-entry sheet **in-app** directly from your
> `onMessage` handler. When **backgrounded**, surface a local notification so the
> guard can tap in. Route both the notification tap and the foreground message to
> the **same** sheet, and **dedupe on `check_id`** so a challenge that arrives both
> ways isn't shown twice. The code is already in `data.code` — nothing to fetch.
>
> **If the push is missed entirely,** poll `GET /shifts/{id}/wakefulness/pending`
> (on foreground + every ~15–20 s while active) to discover the challenge and raise
> the same sheet. Call `/received` when you show it. This is the wakefulness twin of
> the photo `pending` poll — it's what makes the in-app prompt reliable rather than
> push-dependent.
...
> **Foreground rule (same as wakefulness).** If the app is **already open** when the
> request arrives, open the capture flow **in-app** from your `onMessage` handler —
> don't require a notification tap. Route the tap and the foreground message to the
> same flow and dedupe on `request_id`.
...
> **Offline = no push, ever.** A push needs connectivity; an offline device has
> none, so the backend cannot notify it. The **offline prompt is a local
> notification your app schedules** from the provisioned `schedule` — not a server
> push. Backend's offline role is only to provision (below) and to receive the
> result on reconnect (offline-flush endpoint below). Same rule for photos
> (`photo_schedule`). Full breakdown: `FLUTTER_HANDOFF_WAKEFULNESS_OFFLINE_AND_UX.md` §0.
...
- **Answer (online):** `POST /wakefulness/{checkId}/respond` with `{ "code": "4821", "responded_at": "..." }`
  → `{ "data": { "result": "PASSED" | "FAILED" } }`. You get the `checkId` from the
  push (or the pending poll below). See `MOBILE_API_INTEGRATION.md` §6.2.
- **⚠️ Flush offline answers to a DIFFERENT endpoint (Phase 7 — required):** a
  challenge that fired **on-device while offline** has **no server `checkId`** (the
  server never created a row for an unreachable device), so it is **not** sent to
  `respond`. Flush it on reconnect to **`POST /shifts/{id}/wakefulness/offline`**
  with `{ "window_reference": <int>, "code": "4821", "scheduled_at": "...", "responded_at": "..." }`.
  The server re-derives the TOTP for that window and **records the check** (pass or
  fail). **Idempotent** per `(shift, window_reference)` — a re-flush returns
  `reason: "ALREADY_RESOLVED"`; treat any 200 as done and retry only on 5xx/network.
  A wrong code is `FAILED` (200, not an error) and raises **no** retroactive alert.
  **Flush passes and fails alike** — the failures must reach the reports. *Before
  this endpoint existed, offline answers were silently dropped.* Full contract:
  `MOBILE_API_INTEGRATION.md` §6.2.2; deep guidance:
  `FLUTTER_HANDOFF_WAKEFULNESS_OFFLINE_AND_UX.md`.
- **Discover missed online challenges (push fallback):**
  `GET /shifts/{id}/wakefulness/pending` returns any outstanding online challenge
  (`check_id`, `code`, `issued_at`, `response_seconds`, `expires_at`) — the exact
  analogue of the photo `pending` poll. Poll it on foreground (and every ~15–20 s
  while active) so you can raise the code-entry sheet **in-app** even when the push
  was missed. On finding one, call `POST /wakefulness/{checkId}/received` then answer
  via `respond`. See `MOBILE_API_INTEGRATION.md` §6.2.3.
...
---
**📋 2026-07-06 — Offline wakefulness flush + in-app prompts.** **What changed for the app:**
- **New `POST /shifts/{id}/wakefulness/offline`** — flush a wakefulness answer that fired
  **on-device while offline** (no server `checkId`). Body: `{ window_reference, code,
  scheduled_at?, responded_at? }`. The server re-derives the TOTP and records the check
  (pass **or** fail). **Idempotent** per `(shift, window_reference)` → `ALREADY_RESOLVED`;
  treat any 200 as done, retry only 5xx/network. **This was a real gap** — offline answers
  had nowhere to go and were silently dropped; `respond` can't take them (no `checkId`).
  Flush passes *and* fails so the failures reach the timeline/reports. See Wakefulness
  Verification; contract `MOBILE_API_INTEGRATION.md` §6.2.2.
- **New `GET /shifts/{id}/wakefulness/pending`** — push-fallback discovery for online
  challenges (twin of the photo `pending` poll). Poll on foreground / ~15–20 s while active
  to raise the code-entry sheet **in-app** when a push is missed. §6.2.3.
- **Foreground UX (both wakefulness + photo):** when the app is already open, raise the
  prompt **in-app** from `onMessage` — don't wait for a notification tap. Converge the tap
  and foreground handlers on one sheet; dedupe by `check_id` / `request_id`.
- Full guidance: **`FLUTTER_HANDOFF_WAKEFULNESS_OFFLINE_AND_UX.md`**. No change to the
  online `respond` / `received` contract.

---
**📋 2026-06-30 — Phase 7: offline-photo trigger decided (Option A).** This closes the
app's one open Phase 7 question — *what initiates an offline photo capture?* **Answer: a
provisioned photo schedule, analogous to the wakefulness TOTP schedule.** **What changed
for the app:**
- **`POST /shifts/{id}/start` now returns a `photos` block** (next to `wakefulness`) with a
  `schedule` of randomised due-times + `response_seconds` (90, online only) +
  `offline_nonce_ttl_minutes` (15) + `max_photos_per_capture` (5). See **Photo
  Verification → Provisioning**.
- **Offline capture is schedule-triggered:** at each mark, **while offline**, draw a
  pre-fetched pool nonce, capture, queue, flush on reconnect (**omit `request_id`**). While
  **online**, the same mark arrives as a pushed `PHOTO_REQUEST` instead — one source of
  truth, so don't double-fire. Reuse your wakefulness-schedule notifier pattern.
- **No new offline-photo upload contract.** The upload endpoint, signing, nonce pool and
  15-min window are all unchanged from Phase 4 — only the *trigger* is now defined.
- **Backend note:** the server's online photo dispatcher moved from probabilistic firing to
  this shared schedule, so online + offline checks now happen on the same randomised
  cadence. Nothing on the wire changes for the app beyond reading the new `photos` block.
